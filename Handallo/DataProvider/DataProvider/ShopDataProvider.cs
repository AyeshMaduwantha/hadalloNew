    ...
    public class ShopDataProvider : IShopDataProvider
    {
        ...
        public async Task<IActionResult> RegisterShop(Shop shop)
        {
            using (IDbConnection dbConnection = Connection)
            {
                ...
                {
                    string sQuery = "INSERT INTO Shop(ShopName,Des_cription,Email,MobileNo,Lo_cation,OwnerId,Lat,Lng)" +
                                    "VALUES(@ShopName,@Des_cription,@Email,@MobileNo,@Lo_cation,@OwnerId,@Lat,@Lng)";
